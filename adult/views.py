...
def user_owns_list(view_func):
    @wraps(view_func)
    def wrapper(request,pk):
        board = get_object_or_404(Adultmodel,pk=pk)
...
def contact(request):
    if request.method=='POST':
        ...
        if form.is_valid:
            contact=form.save()
            usersubject='お問い合わせ受け付けました'
            usermessage = 'お問い合わせ内容：\n\n{}.format(contact.message)'
            send_mail(usersubject,usermessage,settings.EMAIL_HOST_USER,[contact.email])

            adminsubject='お問い合わせがありました'
            usermessage = 'お問い合わせ内容：\n\n{}.format(contact.message)'
            send_mail(adminsubject,usermessage,settings.EMAIL_HOST_USER,[settings.EMAIL_HOST_USER])
